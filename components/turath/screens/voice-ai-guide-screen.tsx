    ...
    if (isListening) {
      ...
      setTimeout(() => {
        const aiMessage: ChatMessage = {
          ...
          role: 'assistant',
          content: 'The Hassan Tower (صومعة حسان) is the minaret of an incomplete mosque in Rabat, Morocco. Construction began in 1195 under Almohad Caliph Yacoub al-Mansour. At 44 meters tall, it would have been the largest minaret in the world. The tower is now a UNESCO World Heritage Site and remains one of Morocco\'s most iconic landmarks.',
          timestamp: new Date(),
          tags: ['Rabat', 'Architecture', 'UNESCO'],
        }
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      const aiMessage: ChatMessage = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: 'That\'s a great question! Morocco has a rich cultural heritage spanning thousands of years. Let me tell you more about the specific topic you asked about...',
        timestamp: new Date(),
    ...
    <div className="h-full flex flex-col bg-[#0F0F0F] dark">
      ...
      <div className="flex-shrink-0 flex flex-col items-center justify-center py-8">
        <div className="relative">
          {/* Outer pulsing ring */}
          <div className={cn(
            "absolute inset-0 rounded-full border-4 border-[#C9A84C] transition-all duration-500",
            isSpeaking && "gold-pulse"
          )} />
          
          {/* Inner rings */}
          <div className={cn(
            "relative w-32 h-32 rounded-full bg-[#1E1C1A] flex items-center justify-center",
            isListening && "ring-4 ring-[#C1121F]/50"
          )}>
            {/* Waveform bars */}
              ...
              {[1, 2, 3, 4, 5, 4, 3, 2, 1].map((height, i) => (
                <div 
                  ...
                  className={cn(
                    ...
                    isSpeaking || isListening ? "bg-[#C9A84C]" : "bg-[#3A3836]"
                  )}
                  style={{ 
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            >
              <MicIcon className={cn(
                "w-7 h-7",
                isListening ? "text-white" : "text-[#1A1A1A]"
              )} />
            </button>
      ...
      {/* Bottom navigation */}
      <BottomNavigation activeTab="guide" onTabChange={() => {}} />
    </div>
  )
}
