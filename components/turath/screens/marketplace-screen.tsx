...
export function MarketplaceScreen({ onProductSelect, onArtisanSelect, isDark }: MarketplaceScreenProps) {
  ...
  return (
    <div className="h-full flex flex-col">
                ...
                <div className="relative">
                  ...
                  >
                    <HeartIcon 
                      className={cn(
                        "w-4 h-4",
                        likedProducts[product.id] ? "text-primary fill-primary" : "text-muted-foreground"
                      )}
                      filled={likedProducts[product.id]}
                    />
                  </button>
                  {/* Commission badge */}
                  <div className="absolute bottom-2 left-2 px-2 py-1 bg-secondary/90 text-secondary-foreground rounded-lg text-[10px] font-medium">
                    {product.commission}% to artisan
                  </div>
                </div>
                <div className="p-3">
      ...
      {/* Bottom navigation */}
      <BottomNavigation activeTab="artisans" onTabChange={() => {}} />
    </div>
  )
}
